...
INSERT INTO roles VALUES (5, 24, 'Ramona Flowers');
...
-- Affiche tous les titres de film contenant la combinaison de lettres 'the'

SELECT film_titre AS titre_des_films_contenant_chaine_caractere_the
FROM films
WHERE film_titre LIKE '%the%'
ORDER BY film_titre;

-- Affiche le nombre de roles differents present dans la table roles

SELECT COUNT(role_nom) AS nombre_de_roles
FROM roles;

-- Affiche tous les titres de film ainsi que le nombre d'acteurs y jouant un role dans la base de donnees

SELECT films.film_titre, COUNT(roles.acteur_id) AS nombre_acteurs_jouant_role
FROM films
INNER JOIN roles ON roles.film_id = films.film_id
GROUP BY films.film_titre
ORDER BY nombre_acteurs_jouant_role DESC, films.film_titre;

-- Affiche le nom complet des realisateurs et le genre de films qu'ils ont realises

SELECT DISTINCT realisateurs.realisateur_prenom || ' ' || realisateurs.realisateur_nom AS realisateur_nom_complet, genres.genre_nom AS genre_de_films_realises
FROM films
INNER JOIN realisateurs ON realisateurs.realisateur_id = films.realisateur_id
INNER JOIN genres ON genres.genre_id = films.genre_id
ORDER BY realisateur_nom_complet, genre_de_films_realises;

---- Supression des entites

-- Suppression des tables utilisees
...
-- Suppression des sequences utilisees
